...
import {
  descargarPlantillaPendientes,
  useImportPendientesApply,
  useImportPendientesPreview,
} from "@/features/importacion/api/use-import-pendientes"
import type { ImportPendientesPreview } from "@/features/importacion/types"
...
export default function ImportacionPendientesPage() {
  const [archivo, setArchivo] = useState<File | null>(null)
  const [preview, setPreview] = useState<ImportPendientesPreview | null>(null)
  const [applyMensaje, setApplyMensaje] = useState<string | null>(null)
  const [errorGlobal, setErrorGlobal] = useState<string | null>(null)
  const inputRef = useRef<HTMLInputElement>(null)

  const previewMut = useImportPendientesPreview()
  const applyMut = useImportPendientesApply()
  ...
  return (
    <div className="space-y-6 p-6 max-w-5xl">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Importar pendientes</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Carga masiva de pendientes del proyecto desde un archivo Excel. Soporta crear,
          modificar y eliminar (soft-delete) en una sola operación.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Bajá un Excel con los pendientes activos del proyecto. Las filas vienen marcadas
            como SKIP (no se tocan). Cambiá <code className="text-xs">Accion</code> a CREATE
            / UPDATE / DELETE según corresponda, y agregá filas nuevas para crear pendientes.
            Al crear, el pendiente nace en estado <strong>ABIERTO</strong>.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button variant="outline" size="sm" onClick={descargarPlantillaPendientes} className="gap-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Subí el archivo editado. Vas a ver el resumen de qué se va a crear, actualizar
          ...
          <CardContent className="space-y-4">
            <div className="rounded-md border p-3 max-w-sm">
              ...
              <div className="mt-2 flex items-center gap-3 text-sm">
                <span className="text-green-700">+{preview.pendientes.creates}</span>
                <span className="text-blue-700">~{preview.pendientes.updates}</span>
                <span className="text-red-700">−{preview.pendientes.deletes}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Total: {preview.pendientes.total}</p>
            </div>

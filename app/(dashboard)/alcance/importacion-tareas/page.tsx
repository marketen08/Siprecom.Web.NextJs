...
} from "@/features/importacion/api/use-import-tareas"
import type { ImportTareasPreview } from "@/features/importacion/types"
...
export default function ImportacionTareasPage() {
  const [archivo, setArchivo] = useState<File | null>(null)
  const [preview, setPreview] = useState<ImportTareasPreview | null>(null)
  ...
  return (
    <div className="space-y-6 p-6 max-w-5xl">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Importar tareas</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Carga masiva de tareas del proyecto desde un archivo Excel. Soporta crear, modificar y
          eliminar en una sola operación. Las planillas, tipos de elemento, niveles y procedimientos
          referenciados deben existir previamente.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Bajá un Excel con las tareas activas del proyecto. Las filas vienen marcadas como SKIP.
            Cambiá <code className="text-xs">Accion</code> a CREATE / UPDATE / DELETE según
            corresponda. La planilla se referencia por <strong>código</strong> y se usa siempre su
            última versión.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button variant="outline" size="sm" onClick={descargarPlantillaTareas} className="gap-2">
          ...
          <CardContent className="space-y-4">
            <div className="rounded-md border p-3 max-w-sm">
              ...
              <div className="mt-2 flex items-center gap-3 text-sm">
                <span className="text-green-700">+{preview.tareas.creates}</span>
                <span className="text-blue-700">~{preview.tareas.updates}</span>
                <span className="text-red-700">−{preview.tareas.deletes}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Total: {preview.tareas.total}</p>
            </div>

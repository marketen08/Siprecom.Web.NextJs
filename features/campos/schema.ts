...
export const campoSchema = z.object({
  ...
  // 13 (Espacio — bloque vacío display-only).
  tipoDato: z.number().int().min(1).max(13).refine((v) => v !== 6 && v !== 7, { message: "Tipo no válido" }),
  ...
  /** Solo Espacio (tipoDato === 13). Alto en mm. Default 20, rango 5-200. */
